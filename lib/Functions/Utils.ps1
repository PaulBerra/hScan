function LoadConfig {
    ...
    param (
        [string]$configPath = $ConfigObject # to change
    )
...
function BuildReport {
    ...
    $templatesPath = Resolve-Path ("lib\templates\report_csv_summary.ps1")   
...
function YaraDebug {
    ...
    $scanResult
        ...
        Write-Host "File: $(Split-Path -Leaf $scanResult.TargetPath)" -ForegroundColor Yellow
        Write-Host "  Success: $($scanResult.Success)" -ForegroundColor $(if($scanResult.Success){'Green'}else{'Red'})
        Write-Host "  HasDetections: $($scanResult.HasDetections)" -ForegroundColor $(if($scanResult.HasDetections){'Red'}else{'Green'})
        ...
        if ($scanResult.HasDetections) {
            ...
            $scanResult.RuleMatches | ForEach-Object { Write-Host "    Rule: $($_.Rule)" -ForegroundColor Red }
        }
        Write-Host ""
}